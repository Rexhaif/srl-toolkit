...
def get_children(word_num, syntax_dep_tree):
    return [
        child_number
        for child_number, child_syntax in enumerate(syntax_dep_tree)
...
def extract_preposition(arg_number, postags, morph, lemmas, syntax_dep_tree):
    ...
    for child_number in children:
        ...
        if postag_child in PREP_POSTAGS:
            complex_prep = in_complex_preposition(
                child_number, postags, morph, lemmas, syntax_dep_tree
            )
            if complex_prep:
                return complex_prep
            else:
    ...
    for child_number in siblings:
        ...
        if postag_child in PREP_POSTAGS:
            complex_prep = is_complex_preposition(
                child_number, morph, lemmas, syntax_dep_tree
            )
            if complex_prep:
                return complex_prep
            else:
...
def complex_preposition_child(complex_prep, syntax_dep_tree):
    ...
    children = (
        e
        for e in get_children(complex_prep[1], syntax_dep_tree)
